...
void combat_flow::unit_attacks(Entity* source, Entity* target)
{
	...
	if(source->transform->position.x < target->transform->position.x)
		animator::start_animation(ac, animations::attacking_from_left, &attack_animation_finished_callback);
	else
		animator::start_animation(ac, animations::attacking_from_right, &attack_animation_finished_callback);
}
...
void combat_flow::update_quantity_display(Entity* target)
{
	...
	//set to -1 bc get_texture_from_two renders second texture aligned to whole results center if specified to -1
	text_rect.x = -1;
	text_rect.y = -1;
